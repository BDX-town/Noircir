...
export const Password = ({ required = false, ...rest}: { [x: string]: unknown, required?: boolean }) => {
    const root = React.useRef<HTMLFieldSetElement>(null);
    const { T, __ } = useTranslations('Password', { 'fr-FR': fr });

    const [passwordChanged, setPasswordChanged] = React.useState(required);
    ...
    return (
        <fieldset {...rest} ref={root} className='border-0 flex flex-col gap-4 p-0' onChange={() => setPasswordChanged(true)}>
            <div>
